...
%%
addpath('D:\Image Datasets\INRIA\code3.2.1');
clc;clear all;close all;

%% extract training and testing images and ground truth
cd(fileparts(which('acfDemoInria.m'))); dataDir='D:\Image Datasets\INRIA\code3.2.1\data-INRIA\data\';
for s=1:2, pth='D:\Image Datasets\INRIA\code3.2.1\data-INRIA';
...
for it=1:length(imgNms)
I=imread(imgNms{it}); tic, bbs=acfDetect(I,detector); toc
clf; im(I); bbApply('draw',bbs(bbs(:,5)>50,:)); 
drawnow;
input('');
end
...
if( 1 ), bbGt('cropRes',gt,dt,imgNms,'type','fp','n',50,...
