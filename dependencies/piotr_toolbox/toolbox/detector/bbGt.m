...
% Display evaluation results for given image and two detections.
%   [hs,hImg] = bbGt( 'showRes' I, gt, dt1,dt2, varargin )
...
% bbGt>loadAll, bbGt>evalRes, bbGt>showRes, bbGt>showRes2,  bbGt>compRoc, bbGt>cropRes,
...
function [hs,hImg] = showRes2( I, gt, dt1,dt2, varargin )
% Display evaluation results for given image.
%
% USAGE
%  [hs,hImg] = bbGt( 'showRes2', I, gt, dt, varargin )
%
% INPUTS
%  I          - image to display, image filename, or []
%  gt         - first output of evalRes()
...
if( evShow )
  if(gtShow), for i=1:size(gt,1), k=k+1;
      hs{k}=bbApply('draw',gt(i,1:4),cols(gt(i,5)+2),lw,gtLs); end; end
  if(dtShow), for i=1:size(dt1,1), k=k+1;
      hs{k}=bbApply('draw',dt1(i,1:5),cols(dt1(i,6)+2),lw,dtLs); end; end
else
  if(gtShow), k=k+1; hs{k}=bbApply('draw',gt(:,1:4),cols(1),lw,gtLs); end
  if(dtShow), k=k+1; hs{k}=bbApply('draw',dt1(:,1:4),cols(2),lw,dtLs);bbApply('draw',dt2(:,1:4),cols(3),2,dtLs); end
end
hs=[hs{:}]; hold off;
end
...
nImg=size(gt,1); assert(size(dt,1)==nImg);
