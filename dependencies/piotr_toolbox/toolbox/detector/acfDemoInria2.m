...
opts.modelDs=[20 37.5]; opts.modelDsPad=[27 50];
...
imgNms=bbGt('getFiles',{'E:\E\KAUST\GMSV\Datasets\Urban\london\rectified\'});
for it=1:100%length(imgNms)
I=imread(imgNms{it}); tic, bbs=acfDetect(I,detector); toc
clf; im(I); 
I1=rgb2gray(I);
% lsd(I1);
[gx,gy]=imgradient(I1);
...
[subs_h,~] = nonMaxSupr(H,3);
[subs_v,~] = nonMaxSupr(V',3);

plot(H/max(H)*100,'linewidth',2);
plot([subs_h(:,2) subs_h(:,2)],[1 size(I,1)],'b');

plot(V/max(V)*100,1:size(I,1),'red','linewidth',2);
plot([1 size(I,2)],[subs_v(:,2) subs_v(:,2)],'r');
...
if( 1 )
  ...
  tic, parfor i=1:n, acfDetect(Is{i},detector1); end;
