...
imnames=dir([foldername '*.txt']);
for i=1:100
    clf;
    I=imread([imdir imnames(i).name(1:end-4) '.jpg']);
    tic, bbs=edgeBoxes(I,model,opts); toc
    
    
    map=imread([mapdir imnames(i).name(1:end-4) '.png']);
    ...
    bb=dlmread([foldername imnames(i).name(1:end-4) '.txt']);
    bb(1,:)=[];
    bb(:,6)=bb(:,1);
    bb(:,1)=[];
